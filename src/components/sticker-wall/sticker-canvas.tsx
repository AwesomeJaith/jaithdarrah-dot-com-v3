...
export function StickerCanvas({ initialStickers }: StickerCanvasProps) {
  ...
  const [notchSize, setNotchSize] = useState<{ width: number; height: number } | null>(null)
  ...
  // Track notch root size (the motion.div that grows when upload card opens)
  useEffect(() => {
    const root = upload.notchRootRef.current
    if (!root) return
