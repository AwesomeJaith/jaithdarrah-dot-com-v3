...
export function StickerPopup({ sticker }: { sticker: Sticker }) {
  return (
    <div className="pointer-events-none absolute bottom-full left-1/2 z-50 mb-2 -translate-x-1/2 animate-in duration-150 zoom-in-95 fade-in">
      <div className="rounded-lg border border-border bg-popover px-3 py-2 text-sm whitespace-nowrap text-popover-foreground shadow-md">
        <p className="font-medium">{sticker.username}</p>
        {sticker.message && (
